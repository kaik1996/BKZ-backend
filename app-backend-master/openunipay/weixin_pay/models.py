...
class WeiXinQRPayEntity(models.Model):
    ...
    time_stamp = models.CharField(verbose_name=u'时间戳', max_length=10, editable=False)
    product_id = models.CharField(verbose_name=u'商品ID', max_length=32, editable=False)

    class Meta:
        verbose_name = u'微信扫码支付-二维码URL'
...
class WeiXinQRPayRecord(models.Model):
    appid = models.CharField(verbose_name=u'公众账号ID', max_length=32, editable=False)
    mch_id = models.CharField(verbose_name=u'商户号', max_length=32, editable=False)
    openid = models.CharField(verbose_name=u'用户标识', max_length=128, editable=False)
    product_id = models.CharField(verbose_name=u'商品ID', max_length=32, editable=False)

    class Meta:
        verbose_name = u'微信扫码支付-扫码纪录'
        verbose_name_plural = u'微信扫码支付-扫码纪录'
...
class WeiXinPayResult(models.Model):
    order = models.OneToOneField(WeiXinOrder, on_delete=models.CASCADE, primary_key=True, editable=False, related_name='pay_result')
    prepayid = models.CharField(verbose_name=u'预支付交易会话标识', null=True, blank=True, max_length=64, db_index=True, editable=False)
    openid = models.CharField(verbose_name=u'用户标识(openId)', null=True, blank=True, max_length=128, editable=False)
    qrcode = models.CharField(verbose_name=u'二维码链接', null=True, blank=True, max_length=128, editable=False)
    bank_type = models.CharField(verbose_name=u'付款银行', null=True, blank=True, max_length=16, editable=False)
    total_fee = models.PositiveIntegerField(verbose_name=u'总金额', null=True, blank=True, editable=False)
