...
def create_order(weixinOrderObj):
    ...
    def handle_response_qrcode(responseData):
        payResultObj.code_url = responseData['code_url']
        payResultObj.save()
        _logger.info("order created. qrocde is {}".format(payResultObj.code_url))
        return responseData['code_url']
    if weixinOrderObj.trade_type == 'NATIVE':
        return __handle_weixin_api_xml_response(r, handle_response_qrcode)
    else:
        return __handle_weixin_api_xml_response(r, handle_response)

############### 统一关闭订单 #######################
def close_order(weixinOrderObj):
    assert isinstance(weixinOrderObj, WeiXinOrder)
    _logger.info("close order")
