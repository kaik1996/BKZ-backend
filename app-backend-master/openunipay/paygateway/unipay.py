...
@transaction.atomic
def get_order_payway(orderno):
    '''
    @summary: query payway of order
    @param orderno: order no
    '''
    orderItemObj = OrderItem.objects.get(orderno=orderno)
    if orderItemObj.payway == PAY_WAY_WEIXIN:
        trade_type = weixin.WeiXinOrder.objects.get(out_trade_no=orderno).trade_type
    else:
        trade_type = PAY_WAY_ALI
    return (orderItemObj.payway, trade_type)

@transaction.atomic
def close_order(orderno):
    '''
    @summary: close order
    @param orderno: order no
    '''
    orderItemObj = OrderItem.objects.get(orderno=orderno)
    payway = orderItemObj.payway
    result = _PAY_GATEWAY[payway].close_order(orderItemObj)
    return result

def get_pay_info(orderno):
    cache = django_redis.get_redis_connection()
    pay_info = cache.get(orderno)
    if pay_info:
        result = json.loads(pay_info.decode())
        return result
    else:
        return None

def set_pay_info(orderno, result):
    cache = django_redis.get_redis_connection()
    payinfo = json.dumps(result)
    cache.set(orderno, payinfo, 60*5)
    return True
