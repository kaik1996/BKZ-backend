...
class WeiXinPayGateway(PayGateway):
    ...
    def create_order(self, orderItemObj, clientIp, **kwargs):
        ...
        weixinOrderObj.save()
        result = weixin_pay_lib.create_order(weixinOrderObj)
        if weixinOrderObj.trade_type == 'NATIVE':
            data = {
                'qrcode': result,
                'price': orderItemObj.fee
            }
            return data
        data = {'appId': settings.WEIXIN['app_id'],
                ...
                'package': 'prepay_id='+result,
                'timeStamp': str(datetime.get_unix_timestamp())}
        data['sign'] = security.sign(data)
        return data

    def close_order(self, orderItemObj):
        weixinOrderObj = WeiXinOrder.objects.get(out_trade_no=orderItemObj.orderno)
        weixin_pay_lib.close_order(weixinOrderObj)
